...
def prepData_FaceForensics(resultsDir, dataFiles, mustContains=[]):
    ...
    for dataFile in dataFiles:
        ...
        fileNumbers = []
        numEntries = 0
        for fileNumber, filename in enumerate(relevantFiles):
            #print(filename)
            my_data = np.genfromtxt(filename, delimiter=',')
            my_data = my_data.flatten()
    ...
    fileNumbers = np.asarray(allTheFileNumbers).reshape(numFeatures, numEntries).T
    if numFeatures > 1:
        ...
        if not equal:
            ...
            labels = labels[:, 0]
    fileNumbers = fileNumbers[:,0]

    print(data.shape)
    print(labels.shape)

    return data, labels, numFeatures, fileNumbers
...
def majorityVoting(fileNums, labels, preds):
    totalFiles = fileNums[-1] + 1
    print("There are {} files".format(totalFiles))
    rights = [0] * totalFiles
    wrongs = [0] * totalFiles
    for f, l, p in zip(fileNums, labels, preds):
        #print("For file {} of {}".format(f, totalFiles))
        if l == p:
            rights[f] = rights[f] + 1
        else:
            wrongs[f] = wrongs[f] + 1
    ...
    t = [i - j for i, j in zip(rights, wrongs)]
    #print(t)

    wrongImages = sum(1 for i in t if i < 0)
    accuracy = 100*(totalFiles-wrongImages)/totalFiles
    print("There are {} wrong images out of {} by majority voting which is {}".format(wrongImages, totalFiles, accuracy))
    ...
    for example in dataPaths:
        ...
        for dataFiles in dataFilesList:
            ...
            for addBorders in [False]:
                ...
                #                                                dataFiles, keyFramesOnly, addBorders, predFramesOnly)
                data, labels, numFeatures, fileNumbers = prepData_FaceForensics(dataPath, dataFiles)
                trainX, trainY, numFeatures, trainFileNumbers = prepData_FaceForensics(dataPath, dataFiles, ["train"])
                testX, testY, numFeatures, testFileNumbers = prepData_FaceForensics(dataPath, dataFiles, ["test"])
                valX, valY, numFeatures, valFileNumbers = prepData_FaceForensics(dataPath, dataFiles, ["val"])
                print("There are {} train, {} test, {} val".format(len(trainY), len(testY), len(valY)))
                ...
                for balanceType in balanceTypes:
                    ...
                    for analysisType in analysisTypes:
                        ...
                        print("Doing analysis with {} and test set".format(analysisType))
                        ...
                        tuple = [filebasename, featureCode, balanceCode, classifierCode, 0, cm[0,0], cm[0,1], cm[1,0], cm[1,1], accuracy, total_f1s]
                        resultsList.append(tuple)
                        writer.writerow(tuple)
                        f.flush()
                        print(tuple)

                        majorityVoting(testFileNumbers, testY, predictions)

                        print("Doing analysis with {} and val set".format(analysisType))
                        print(classification_report(valY, predictions_val))
                        cm = confusion_matrix(valY, predictions_val)
                        print(cm)
                        accuracy = accuracy_score(valY, predictions_val)
                        print(accuracy)
                        f1s = f1_score(valY, predictions_val, average=None)
                        total_f1s = np.sum(f1s)
                        print("Total f1 over both classes: {}".format(total_f1s))
                        featureCode = getFeatureCode(dataFiles, addBorders)
                        balanceCode = getBalanceCode(balanceType)
                        classifierCode = getClassifierCode(analysisType)
                        tuple = [filebasename, featureCode, balanceCode, classifierCode, 1, cm[0,0], cm[0,1], cm[1,0], cm[1,1], accuracy, total_f1s]
                        resultsList.append(tuple)
                        writer.writerow(tuple)
                        f.flush()
                        print(tuple)

                        majorityVoting(valFileNumbers, valY, predictions_val)
