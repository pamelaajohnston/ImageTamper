    ...
    for example in dataPaths:
        ...
        for dataFiles in dataFilesList:
            ...
            for addBorders in [False]:
                ...
                trainX, trainY, numFeatures = prepData_FaceForensics(dataPath, dataFiles, ["train"])
                testX, testY, numFeatures = prepData_FaceForensics(dataPath, dataFiles, ["test"])
                valX, valY, numFeatures = prepData_FaceForensics(dataPath, dataFiles, ["val"])
                ...
                for balanceType in balanceTypes:
                    ...
                    for analysisType in analysisTypes:
                        ...
                        balanceCode = getBalanceCode(balanceType)
                        classifierCode = getClassifierCode(analysisType)
                        tuple = [filebasename, featureCode, balanceCode, classifierCode, 1, cm[0, 0], cm[0, 1],
                                 cm[1, 0], cm[1, 1], accuracy, total_f1s]
                        resultsList.append(tuple)
                        writer.writerow(tuple)
                        f.flush()
