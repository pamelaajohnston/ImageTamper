...
# then uses the model to make predictions
    ...
    #return data, labels, numFeatures
    return predictions, labels, numFeatures
...
def analyseData(data, labels, dataFiles, balance=True, ubername = ""):
    ...
    if doKmeans:
        if fudged == 0:
            #mydata, mylabs = balanceData("SMOTE", data, labels)
            mydata = data
            mylabs = labels
            model = KMeans(n_clusters=2)
            model.fit(mydata)
            predictions = model.predict(mydata)
            print(classification_report(mylabs, predictions))
            cm = confusion_matrix(mylabs, predictions)
            print(cm)
            accuracy = accuracy_score(mylabs, predictions)
            print("Accuracy is: {}".format(accuracy))
            f1s = f1_score(mylabs, predictions, average=None)
            ...
            tpr = tp/(tp+fn)
            tnr = tn/(tn+fp)
            print("tp, fp, tn, fn, mcc, tpr, tnr")
            print("{} {} {} {} {} {}".format(tp, fp, tn, fn, mcc, tpr, tnr))
    ...
    for i, dataFile in enumerate(dataFiles):
        ...
        mask0_w = np.empty(plot0data.shape)
        mask0_w.fill(1 / plot0data.shape[0])
        mask1_w = np.empty(plot1data.shape)
        mask1_w.fill(1 / plot1data.shape[0])

        n0, bins0, patches0 = plt.hist([plot0data, plot1data],
                                       bins=binrange,
                                       color=['#fffb00', '#000000'],
        ...
        print("Saving graph as:   {}fig_FaceForensics_{}.png".format(ubername, name))
        plt.show()
...
if __name__ == "__main__":
    ...
    onlyFaceForensics = True
    doSpecificTests = True
    ...
    for example in dataPaths:
        ...
        analysisTypes = ["ANN", "SVM", "Linear Regression", "Decision Tree", "Random Forest", "Naive Bayes", "kmeans"]
        ...
        for dataFiles in dataFilesList:
            ...
            for addBorders in [False]:
                ...
                #analyseData(trainX, trainY, dataFiles)
                #analyseData(testX, testY, dataFiles)
                #analyseData(valX, valY, dataFiles)
                analyseData(testX_fullFrame, testY_fullFrame, dataFiles)
                #VTD_dahuaX, VTD_dahuaY, numFeatures = prepData_Davino(dataFiles, 5) # This line for thesis graphs
                #analyseData(VTD_dahuaX, VTD_dahuaY, dataFiles)
                #quit()
                ...
                for balanceType in balanceTypes:
                    ...
                    for analysisType in analysisTypes:
                        if analysisType == "ANN":
                            ...
                            #newModel = KMeans(n_clusters=2)
                            newModel = AffinityPropagation()
                            model = newModel
                            model.fit(testX)
                            ...
                            predictFiles(
                                "/Volumes/LaCie/data/FaceForensics/FullResults_fullFrame_selected/",
                                dataFiles,
                                model,
                                [])
                            if doSpecificTests:
                                predictFiles("/Volumes/LaCie/data/FaceForensics/FullResults_fullFrame/1aJO2VkfZiY_2_EMLALfhSftA_0_oneFrame_640x480_test_altered", dataFiles,
                                             model,
                                             ["test"])
                                predictFiles("/Volumes/LaCie/data/FaceForensics/FullResults_fullFrame/1aJO2VkfZiY_2_EMLALfhSftA_0_oneFrame_640x480_test_original", dataFiles,
                                             model,
                                             ["test"])
                                predictFiles("/Volumes/LaCie/data/FaceForensics/FullResults_fullFrame/v_MP513a0v8_0_vjSgViWZEjY_2_oneFrame_640x480_test_altered", dataFiles,
                                             model,
                                             ["test"])
                                predictFiles("/Volumes/LaCie/data/FaceForensics/FullResults_fullFrame/v_MP513a0v8_0_vjSgViWZEjY_2_oneFrame_640x480_test_original", dataFiles,
                                             model,
                                             ["test"])
                                predictFiles("/Volumes/LaCie/data/FaceForensics/FullResults_fullFrame/BEmHm2TaUDA_1_DdhwFUtoKzk_0_oneFrame_640x480_test_altered", dataFiles,
                                             model,
                                             ["test"])
                                predictFiles("/Volumes/LaCie/data/FaceForensics/FullResults_fullFrame/BEmHm2TaUDA_1_DdhwFUtoKzk_0_oneFrame_640x480_test_original", dataFiles,
                                             model,
                                             ["test"])
